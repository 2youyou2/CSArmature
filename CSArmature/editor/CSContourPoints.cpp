...
ContourPointType ContourPoints::containWorldPoint(cocos2d::CCPoint &_point)
{
    ...
    do
    {
		...
		if(CS_SPRITE_CONTAIN_POINT_WITH_RETURN(m_pLeftDown, p, outP))
		{
		...
		if(CS_SPRITE_CONTAIN_POINT_WITH_RETURN(m_pLeftUp, p, outP))
		{
			CC_BREAK_IF(m_pTextureMutable->pixelAt(outP).a != 0);
			return m_eContourPointType = POINT_MIN;
		}
		...
		if(CS_SPRITE_CONTAIN_POINT_WITH_RETURN(m_pRightDown, p, outP))
		{
			CC_BREAK_IF(m_pTextureMutable->pixelAt(outP).a != 0);
			return m_eContourPointType = POINT_MIN;
		}
		...
		if(CS_SPRITE_CONTAIN_POINT_WITH_RETURN(m_pRightUp, p, outP))
		{
			CC_BREAK_IF(m_pTextureMutable->pixelAt(outP).a != 0);
			return m_eContourPointType = POINT_MIN;
		}
		...
		if(CS_SPRITE_CONTAIN_POINT_WITH_RETURN(m_pCenter, p, outP))
		{
			CC_BREAK_IF(m_pTextureMutable->pixelAt(outP).a != 0);
			return m_eContourPointType = POINT_MIN;
		}
		...
		if(CS_SPRITE_CONTAIN_POINT_WITH_RETURN(m_pRotation, p, outP))
		{
			CC_BREAK_IF(m_pTextureMutable->pixelAt(outP).a != 0);
			return m_eContourPointType = POINT_MIN;
		}
